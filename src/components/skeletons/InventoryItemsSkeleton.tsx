...
interface InventoryItemsSkeletonProps {
  count?: number;
}

export default function InventoryItemsSkeleton({ count = 4 }: InventoryItemsSkeletonProps) {
  return (
    <div className="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
      {Array.from({ length: count }).map((_, index) => (
        <SkeletonBase
          key={index}
          className="rounded-xl sm:rounded-2xl overflow-hidden border border-slate-700/50"
        >
