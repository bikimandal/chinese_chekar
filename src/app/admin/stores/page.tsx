...
export default function StoresPage() {
  ...
  const checkAdminAccess = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Check if user is admin (you'll need to add role to session response)
  ...
  const fetchStores = async () => {
    try {
      // Try to fetch all stores (admin only)
      ...
      if (response.ok) {
        ...
        // Not admin, fetch accessible stores only
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-semibold rounded-lg hover:from-amber-500 hover:to-orange-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    <>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {stores.map((store) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm p-6 rounded-xl border border-slate-700/50 hover:border-amber-500/30 transition-all"
            >
