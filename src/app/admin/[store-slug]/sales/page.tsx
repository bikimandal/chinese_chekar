...
export default function SalesPage() {
  ...
  const generateSalesReportPDF = async () => {
    ...
    try {
      // Fetch store details with invoice information
      let storeDetails = null;
      if (currentStore) {
        try {
          const storeResponse = await fetch(`/api/stores/${currentStore.id}`, {
            credentials: "include",
          });
          if (storeResponse.ok) {
            storeDetails = await storeResponse.json();
          }
        } catch (error) {
      ...
      // Get store invoice details or fallback to footer config
      const storeName = storeDetails?.invoiceName || storeDetails?.name || footerConfig.brand.name;
      const storeAddress = storeDetails?.invoiceAddress || footerConfig.contact.address;
      const storePhone = storeDetails?.invoicePhone || footerConfig.contact.phone;
      ...
      // Header - Use store invoice details
      pdf.setFontSize(20);
      pdf.setFont("helvetica", "bold");
      pdf.text(storeName.toUpperCase(), pageWidth / 2, yPos, {
        align: "center",
      });
      yPos += 6;
      ...
      yPos += 2;
      pdf.text(`Phone: ${storePhone}`, pageWidth / 2, yPos, {
        align: "center",
      });
      yPos += 6;
  ...
  if (isCheckingAuth || storeLoading) {
    return <Loader message="Checking authentication and loading store..." />;
  }
  ...
  const dashboardPath = currentStore ? `/admin/${currentStore.slug}` : "/admin";
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  disabled={loading}
                  className="px-3 sm:px-4 py-1.5 sm:py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 rounded-lg transition-all duration-300 text-xs sm:text-sm font-medium cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                >
                  {loading && selectedDate === new Date().toISOString().split("T")[0] ? (
                    <>
